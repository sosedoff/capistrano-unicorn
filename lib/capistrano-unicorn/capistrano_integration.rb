    ...
    def self.load_into(capistrano_config)
      capistrano_config.load do
        ...
        namespace :unicorn do
          ...
          task :start, :roles => task_roles, :except => {:no_release => true} do
            run start_unicorn
          end

          desc 'Stop Unicorn'
          task :stop, :roles => task_roles, :except => {:no_release => true} do
            run kill_unicorn('QUIT')
          end

          desc 'Immediately shutdown Unicorn'
          task :shutdown, :roles => task_roles, :except => {:no_release => true} do
            run kill_unicorn('TERM')
          end

          desc 'Restart Unicorn'
          task :restart, :roles => task_roles, :except => {:no_release => true} do
            run <<-END
              #{duplicate_unicorn}
          ...
          desc 'Reload Unicorn'
          task :reload, :roles => task_roles, :except => {:no_release => true} do
            run <<-END
              set -x;
              if #{unicorn_is_running?}; then
          ...
          task :add_worker, :roles => task_roles, :except => {:no_release => true} do
            run <<-END
              set -x;
              if #{unicorn_is_running?}; then
          ...
          task :remove_worker, :roles => task_roles, :except => {:no_release => true} do
            run <<-END
              set -x;
              if #{unicorn_is_running?}; then
